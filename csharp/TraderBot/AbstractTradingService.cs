...
public class AbstractTradingService : BackgroundService
{
    ...
    protected readonly ITradingApiProvider ApiProvider;
    protected readonly ILogger<AbstractTradingService> Logger;
    protected readonly IHostApplicationLifetime Lifetime;
    protected readonly TradingSettings Settings;
    protected readonly IAccount CurrentAccount;
    ...
    protected readonly ConcurrentDictionary<string, IOrder> ActiveBuyOrders;
    protected readonly ConcurrentDictionary<string, IOrder> ActiveSellOrders;
    ...
    public AbstractTradingService(ILogger<AbstractTradingService> logger, ITradingApiProvider apiProvider, IHostApplicationLifetime lifetime, TradingSettings settings)
    {
        Logger = logger;
        ...
        Task.Run(async () =>
        {
            await ApiProvider.InitializeAsync();

            var accounts = await ApiProvider.GetAccountsAsync();
            var accountList = accounts.ToList();
            Logger.LogInformation("Accounts:");
            for (int i = 0; i < accountList.Count; i++)
            {
                Logger.LogInformation($"[{i}]: {accountList[i]}");
            }
            if (settings.AccountIndex < 0 || settings.AccountIndex >= accountList.Count)
            {
                throw new ArgumentException($"Account index {settings.AccountIndex} is out of range. Please select a valid account index ({0}-{accountList.Count - 1}).");
            }
            CurrentAccount = accountList[settings.AccountIndex];
            Logger.LogInformation($"CurrentAccount (with {settings.AccountIndex} index): {CurrentAccount}");

            var instrumentType = settings.Instrument == Instrument.Etf ? InstrumentType.Etf : InstrumentType.Shares;
            var currentInstrument = await ApiProvider.GetInstrumentAsync(settings.Ticker!, instrumentType);
            
            Logger.LogInformation($"CurrentInstrument: {currentInstrument}");
            Figi = currentInstrument.Figi;
            Logger.LogInformation($"Figi: {Figi}");
            PriceStep = currentInstrument.MinPriceIncrement;
            Logger.LogInformation($"PriceStep: {PriceStep}");
            LotSize = currentInstrument.Lot;
            Logger.LogInformation($"LotSize: {LotSize}");
        }).Wait();

        ActiveBuyOrders = new ConcurrentDictionary<string, IOrder>();
        ActiveSellOrders = new ConcurrentDictionary<string, IOrder>();
    ...
    {
        var tradesStream = await ApiProvider.SubscribeToTradesAsync(CurrentAccount.Id);
        await foreach (var data in tradesStream)
        {
            Logger.LogInformation($"Trade: {data}");
            if (data.DataType == TradeDataType.OrderTrades && data.OrderTrades != null)
            {
            ...
            else if (data.DataType == TradeDataType.Ping)
            {
                SyncActiveOrders();
                SyncLots();
            }
    ...
    protected void UpdateCashBalance(IOrderTrades orderTrades)
    {
    ...
    protected async void SyncActiveOrders(bool forceReset = false)
    {
        if (forceReset)
        {
        ...
        var orders = await ApiProvider.GetOrdersAsync(CurrentAccount.Id);
        var ordersList = orders.ToList();
        
        var deletedBuyOrders = new List<string>();
        foreach (var order in ActiveBuyOrders)
        {
            if (ordersList.All(o => o.OrderId != order.Key))
            {
        ...
        foreach (var orderState in ordersList)
        {
        ...
        {
            ActiveBuyOrders.TryRemove(orderId, out IOrder? orderState);
        }
        foreach (var orderId in deletedSellOrders)
        {
            ActiveSellOrders.TryRemove(orderId, out IOrder? orderState);
    ...
    protected async void SyncLots(bool forceReset = false)
    {
        if (forceReset)
        {
            LotsSets.Clear();
        ...
        // Get positions
        var positions = await ApiProvider.GetPositionsAsync(CurrentAccount.Id);
        var currentInstrumentPosition = positions.Where(p => p.Figi == Figi).FirstOrDefault();
        ...
        // Get portfolio
        var portfolio = await ApiProvider.GetPortfolioAsync(CurrentAccount.Id);
        var currentInstrumentPortfolio = portfolio.Positions.Where(p => p.Figi == Figi).FirstOrDefault();
        ...
        var openOperations = await GetOpenOperationsAsync();
    ...
    protected void TrySubtractTradesFromOrder(ConcurrentDictionary<string, IOrder> orders, IOrderTrades orderTrades)
    {
    ...
    protected void TryUpdateLots(IOrderTrades orderTrades)
    {
    ...
    {
        var orderBookStream = await ApiProvider.SubscribeToOrderBookAsync(Figi, Settings.MarketOrderBookDepth);
        await foreach (var orderBook in orderBookStream)
        {
            if (cancellationToken.IsCancellationRequested)
                break;
            ...
            var bestBid = bestBidOrder.Price;
            ...
            var bestAsk = bestAskOrder.Price;
            ...
            else if (ActiveBuyOrders.Count == 1)
            {
                ...
                if (IsTimeToBuy())
                {
                    var initialOrderPrice = activeBuyOrder.InitialSecurityPrice;
            ...
            else if (ActiveSellOrders.Count == 1)
            {
                ...
                if (ActiveSellOrderSourcePrice.TryGetValue(activeSellOrder.OrderId, out var sourcePrice))
                {
                    ...
                    if (topBid <= sourcePrice && topBid >= minimumSellPrice && topBidOrder.Quantity < (Settings.EarlySellOwnedLotsDelta + activeSellOrder.LotsRequested * Settings.EarlySellOwnedLotsMultiplier))
                    {
                    ...
                    {
                        var initialOrderPrice = activeSellOrder.InitialSecurityPrice;
    ...
    {
        var portfolio = await ApiProvider.GetPortfolioAsync(CurrentAccount.Id);
        var balanceFree = portfolio.Money.Any() ? portfolio.Money.First(m => m.Currency == Settings.CashCurrency).Value : 0;
        var balanceLocked = portfolio.Blocked.Any() ? portfolio.Blocked.First(m => m.Currency == Settings.CashCurrency).Value : 0;
    ...
    private async Task<OperationsList> GetOpenOperationsAsync()
    {
        DateTime accountOpenDate = DateTime.SpecifyKind(CurrentAccount.OpenedDate, DateTimeKind.Utc).AddHours(-3);
        ...
        var operations = await ApiProvider.GetOperationsAsync(CurrentAccount.Id, Figi, from, DateTime.UtcNow.AddDays(4));
        var operationsList = operations.Select(o => (o.OperationType, o.Date, o.Quantity, o.Price)).OrderBy(x => x.Date).ToList();

        // Log operations
        foreach (var operation in operationsList)
        {
            Logger.LogInformation($"{operation.OperationType} operation with {operation.Quantity} lots at {operation.Price} price on {operation.Date.ToString("o", System.Globalization.CultureInfo.InvariantCulture)}.");
        }

        if (operationsList.Any() && operationsList.First().OperationType == OperationType.Sell)
        {
        ...
        var totalSoldQuantity = operationsList.Where(o => o.OperationType == OperationType.Sell).Sum(o => o.Quantity);
        Logger.LogInformation($"Total sell operations quantity {totalSoldQuantity}");

        var openOperations = operationsList.Where(o => o.OperationType == OperationType.Buy).ToList();
        ...
        {
            var baseDate = operationsList.Last().Date.AddMilliseconds(1);
        ...
        for (var i = 0; totalSoldQuantity > 0 && i < openOperations.Count; i++)
        {
            ...
            if (totalSoldQuantity < actualQuantity)
            {
                ...
                openOperations[i] = (openOperation.OperationType, openOperation.Date, actualQuantity - totalSoldQuantity, openOperation.Price);
    ...
    private async Task<IOrderResponse> PlaceSellOrder(long amount, decimal price)
    {
        var sellOrderRequest = new TinkoffOrderRequest
        {
        ...
        var response = await ApiProvider.PlaceOrderAsync(sellOrderRequest);
        Logger.LogInformation($"Sell order placed: {response}");
        return response;
    }

    private async Task<IOrderResponse> PlaceBuyOrder(long amount, decimal price)
    {
        var buyOrderRequest = new TinkoffOrderRequest
        {
            OrderId = Guid.NewGuid().ToString(),
            AccountId = CurrentAccount.Id,
        ...
        var response = await ApiProvider.PlaceOrderAsync(buyOrderRequest);
    ...
    private async Task<ICancelOrderResponse> CancelOrder(string orderId)
    {
        var response = await ApiProvider.CancelOrderAsync(CurrentAccount.Id, orderId);
