...
pub struct ChangeLicenseInConanfilePyArgument<'a> {
    ...
    pub new_license: &'a String,
    pub commit_message: &'a String,
}

pub async fn change_license_in_conanfile_py(
    argument: &ChangeLicenseInConanfilePyArgument<'_>,
) -> anyhow::Result<()> {
    let ChangeLicenseInConanfilePyArgument {
        source_repo_name,
    ...
    // Regex to match license field - handles both quoted and unquoted licenses
    let regex = RegexBuilder::new(r#"^(?P<indent>\s*)license\s*=\s*(?P<quote>["']?)([^"'\n]+)(?P=quote)(?P<rest>.*)$"#)
        .multi_line(true)
        .build()
        ?;
    ...
    let new_content = regex.replace(&decoded_content, |caps: &Captures| {
        ...
        let quote = caps.name("quote").unwrap().as_str();
        let rest = caps.name("rest").unwrap().as_str();
        
        format!(
            "{indent}license = {quote}{new_license}{quote}{rest}",
            indent = indent,
            quote = quote,
